...
create_fuzz_dir() {
    local NAXSOFTWARE_ROOT=${WORKDIR}/NaxSoftware
    if [ -d "${NAXSOFTWARE_ROOT}" ]; then
      return 0
    fi
    git clone https://github.com/SpinalHDL/NaxSoftware ${NAXSOFTWARE_ROOT}

    mkdir ${NAXSOFTWARE_ROOT}/baremetal/fuzz
    cat > ${NAXSOFTWARE_ROOT}/baremetal/fuzz/makefile << EOF
PROJ_NAME=fuzz

include ../common/asm.mk
EOF
    mkdir ${NAXSOFTWARE_ROOT}/baremetal/fuzz/src
}

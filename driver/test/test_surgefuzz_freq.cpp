...
void test_surgefuzz_freq() {
    ...
    if ((setenv("FUZZ_METHOD_TYPE", "SurgeFuzz", 1) != 0) ||
        (setenv("FUZZ_ANNOTATION_TYPE", "FREQ", 1) != 0) ||
    ...
    SurgeFuzzDriver driver;
    for (std::size_t i = 0; i < cycles; i++) {
        struct CoreType core = {coverage_signal_list[i],
                                annotated_signal_list[i]};
        driver.call_per_one_cycle(&core);
    }

    std::cout << std::endl << "Starting SurgeFuzz(FREQ) tests..." << std::endl;
    ...
    for (std::size_t i = 0; i < cycles; i++) {
        std::uint32_t freq_score = 0;
        for (std::size_t j = 0; j <= i; j++) {
            freq_score += annotated_signal_list[j];
        }
        expected.insert((coverage_signal_list[i] << 4) | freq_score);
    }
    for (std::size_t i = 0; i < bitmap_size; i++) {
